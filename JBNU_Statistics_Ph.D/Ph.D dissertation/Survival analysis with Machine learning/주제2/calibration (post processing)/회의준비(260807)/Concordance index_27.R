...
lambda2 <- 1000 # C ~ Uniform(0, lambda2). 이 값을 줄이면 중도절단율이 높아지고, 늘리면 낮아집니다.
...
time.sort0 <- c(1.578192, 5.416312, 13.177687, 22.983575, 
                28.395851, 32.750467, 37.345431, 47.272982, 
                93.786251, 163.198885, 354.002373, 556.044253)
...
res05 <- my.gradient.descent.U.pair(So, X, alpha=0.005, lambda=10, time.sort0)
res05$beta.new
risk.score_05 <- exp(X%*%res05$beta.new)
C_Index_beta(So, res05$beta.new, X)
C_tilde_beta(So, res05$beta.new, X)
