...
res1 <- my.gradient.descent.U(So, X, alpha=0.01, lambda=0, time.sort4)
res1$beta.new
risk.score_01 <- exp(X%*%res1$beta.new)
...
res2 <- my.gradient.descent.U(So, X, alpha=0.01, lambda=50, time.sort4)
res2$beta.new
risk.score_02 <- exp(X%*%res2$beta.new)
...
res3 <- my.gradient.descent.U(So, X, alpha=0.01, lambda=100, time.sort4)
res3$beta.new
risk.score_03 <- exp(X%*%res3$beta.new)
...
res4 <- my.gradient.descent.U(So, X, alpha=0.2, lambda=5, time.sort4)
res4$beta.new
risk.score_04 <- exp(X%*%res4$beta.new)
...
res5 <- my.gradient.descent.U(So, X, alpha=0.2, lambda=10, time.sort4)
res5$beta.new
risk.score_05 <- exp(X%*%res5$beta.new)
...
ggplot(dt2, aes(x=time, y=AUC, group=case, color=case)) +
  ...
  geom_vline(xintercept=time.sort4, col="darkgreen", linetype="dashed") +
