...
res1 <- my.gradient.descent.U(So, X, alpha=0.01, lambda=0, time.sort5)
res1$beta.new
risk.score_01 <- exp(X%*%res1$beta.new)
...
res2 <- my.gradient.descent.U(So, X, alpha=0.01, lambda=50, time.sort5)
res2$beta.new
risk.score_02 <- exp(X%*%res2$beta.new)
...
res3 <- my.gradient.descent.U(So, X, alpha=0.01, lambda=100, time.sort5)
res3$beta.new
risk.score_03 <- exp(X%*%res3$beta.new)
...
res4 <- my.gradient.descent.U(So, X, alpha=0.2, lambda=5, time.sort5)
res4$beta.new
risk.score_04 <- exp(X%*%res4$beta.new)
...
res5 <- my.gradient.descent.U(So, X, alpha=0.2, lambda=10, time.sort5)
res5$beta.new
risk.score_05 <- exp(X%*%res5$beta.new)
...
ggplot(dt2, aes(x=time, y=AUC, group=case, color=case)) +
  ...
  geom_vline(xintercept=time.sort5, col="darkgreen", linetype="dashed") +
  ggtitle("Time dependent AUC with risk score (gradient descent)") +
  xlab("time") + ylab("AUC") +
  #scale_x_continuous(limits = c(0, 200)) +
  #scale_y_continuous(limits = c(0.65, 0.875)) +
